...
class MakeDataSet:
    ...
    def get_patient_diagnosis(self, pid):
        if pid not in metastatic_data['patient_id'].values:
            return 0;
        else:
            return metastatic_data[metastatic_data['patient_id'] == pid]['patient_diagnosis'].values[0];
        
        
    def save_meta(self,meta_list):
        """Saves the information of nodule to csv file"""
        tmp = pd.Series(meta_list,index=['patient_id','nodule_no','slice_no','original_image','mask_image','malignancy','is_cancer','is_clean', 'patient_diagnosis'])
    ...
    def prepare_dataset(self):
        ...
        for patient in tqdm(self.IDRI_list):
            ...
            patient_diagnosis = self.get_patient_diagnosis(pid)
            ...
            if len(nodules_annotation) > 0:
                ...
                # This current for loop iterates over total number of nodules in a single patient
                    if len(nodule) > 2:
                        ...
                        for nodule_slice in range(mask.shape[2]):
                            ...
                            meta_list = [pid[-4:],nodule_idx,prefix[nodule_slice],nodule_name,mask_name,malignancy,cancer_label,False, patient_diagnosis]
                            
                            # get mask slice
                            mask_slice = mask[:,:,nodule_slice].astype(int) * patient_diagnosis

                            self.save_meta(meta_list)
                            np.save(patient_image_dir / nodule_name,lung_segmented_np_array)
                            np.save(patient_mask_dir / mask_name, mask_slice)
            else:
                ...
                for slice in range(vol.shape[2]):
                    ...
                    meta_list = [pid[-4:],slice,prefix[slice],nodule_name,mask_name,0,False,True,0]
                    self.save_meta(meta_list)
...
if __name__ == '__main__':
    ...
    LIDC_IDRI_list= [f for f in os.listdir(DICOM_DIR) if not f.startswith('.') and f != 'LICENSE']
    LIDC_IDRI_list.sort()
